...
class TestSession(Session):
    ...
    def run_session(self, *args):
        ...
        if 'TFNUM_THREADS' in os.environ:
            TFNUM_THREADS=int(os.environ['TFNUM_THREADS'])
        else:
            TFNUM_THREADS=3

        print("DM testSession: TFNUM_THREADS=%s"% str(TFNUM_THREADS))
        gpu_options = tf.GPUOptions(allow_growth = True)
        with tf.Session( graph=graphTf, config=tf.ConfigProto(gpu_options=gpu_options,intra_op_parallelism_threads=TFNUM_THREADS, inter_op_parallelism_threads=TFNUM_THREADS, log_device_placement=False, device_count={'CPU':999, 'GPU':99}) ) as sessionTf:
